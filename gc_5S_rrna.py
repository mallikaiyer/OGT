...
'''
Input: Path to dir containing all genomes and path to db
...
genbank ID, it grabs all 5S rRNA sequences, and keeps a running count of 
...
writes it to the DB.
Run as follows: ./gc_5S_rrna_publish.py path/to/genomes/dir path/to/db''' 
...
if __name__ == '__main__':
    ...
    query = "SELECT GOLD_ID FROM GOLD_FEATURES WHERE OGT IS NOT NULL AND rRNA_5S_GC IS NULL;"
    c.execute(query)
    ...
    for goldID in goldIDs:
        ...
        if base_count == 0:
            ...
            query = "UPDATE GOLD_FEATURES SET rRNA_5S_GC = {} WHERE GOLD_ID = '{}';".format(frac_gc, goldID)
            c.execute(query)
            conn.commit()
